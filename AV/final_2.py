...
VIDEO_PATH       = os.path.join(AV_DIR, "recorded_clip.mp4")        # <-- video (frames source)
AUDIO_PATH       = os.path.join(AV_DIR, "recorded_audio.wav")       # <-- SEPARATE audio recording
...
def audio_in_window(full_wav, t0, t1):
    ...
    `t0`/`t1` are on the VIDEO timeline; AV_OFFSET_S maps them onto the
    separately-recorded audio timeline.
    """
    a_t0 = t0 + AV_OFFSET_S
    a_t1 = t1 + AV_OFFSET_S
    s0 = max(0, int(a_t0 * SAMPLE_RATE))
    s1 = max(s0, int(a_t1 * SAMPLE_RATE))
    seg = full_wav[s0:s1]
...
if AUDIO_MODE not in ("separate", "combined"):
    raise ValueError(f"AUDIO_MODE must be 'separate' or 'combined', got {AUDIO_MODE!r}.")

if not os.path.exists(VIDEO_PATH):
    raise FileNotFoundError(f"Video not found: {VIDEO_PATH}  (set VIDEO_PATH).")

# choose where the audio comes from based on the mode
audio_src = AUDIO_PATH if AUDIO_MODE == "separate" else VIDEO_PATH
if AUDIO_MODE == "separate" and not os.path.exists(AUDIO_PATH):
    raise FileNotFoundError(f"Audio not found: {AUDIO_PATH}  (set AUDIO_PATH or use AUDIO_MODE='combined').")

# load the audio track (librosa decodes wav/m4a/mp4 via ffmpeg)
try:
    full_wav, _ = librosa.load(audio_src, sr=SAMPLE_RATE, mono=True)
except Exception as e:
    raise RuntimeError(f"Could not read audio from {audio_src}: {e}\n"
...
print(f"\nRecording (mode={AUDIO_MODE}):")
print(f"  video: {VIDEO_PATH}")
print(f"  audio: {audio_src}  (AV_OFFSET_S={AV_OFFSET_S})")
